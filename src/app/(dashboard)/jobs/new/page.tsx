...
function parsePairsToForm(data: Record<string, string> | null | undefined): { key: string; value: string }[] {
  if (!data) return [];
...
export default function NewJobPage() {
      ...
      if (hasBody) {
        if (form.bodyType === "json") {
          if (form.jsonBody.trim()) {
            try { body = JSON.parse(form.jsonBody); } catch { showToast("Invalid JSON body", "error"); setLoading(false); return; }
          }
  ...
  const hasBodyOptions = ["POST", "PUT", "PATCH"].includes(form.method);
