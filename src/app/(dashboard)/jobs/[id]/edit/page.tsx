...
export default function EditJobPage() {
  ...
  const fetchJob = useCallback(async () => {
    try {
      ...
      setForm({
        name: job.name, url: job.url, method: job.method,
        headers: job.headers ? JSON.stringify(job.headers, null, 2) : "",
        body: job.body ? JSON.stringify(job.body, null, 2) : "",
        schedule: job.schedule, isActive: job.isActive,
        timeout: job.timeout, retryCount: job.retryCount,
      });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (form.headers.trim()) {
        try { headers = JSON.parse(form.headers); } catch { showToast("Invalid headers JSON", "error"); setSaving(false); return; }
      }
      let body = null;
      if (form.body.trim() && form.method !== "GET") {
        try { body = JSON.parse(form.body); } catch { showToast("Invalid body JSON", "error"); setSaving(false); return; }
      }
      const res = await fetch("/api/jobs/" + params.id, {
        method: "PUT",
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
            ...
            <div>
              ...
              <textarea value={form.headers} onChange={(e) => setForm({ ...form, headers: e.target.value })}
                className={inputCls + " font-mono text-xs"} rows={3} placeholder='{"Authorization": "Bearer token"}' />
            </div>
            {form.method !== "GET" && (
              <div>
            ...
            {!useCustomSchedule ? (
              ...
              <div>
                ...
                <button type="button" onClick={() => setUseCustomSchedule(false)}
                  className="text-sm text-brand-600 hover:text-brand-700 font-medium mt-2">Use preset schedule</button>
              </div>
            )}
          ...
          <div className="flex justify-end gap-3">
            <button type="button" onClick={() => router.back()}
              className="px-5 py-2.5 text-sm font-semibold text-gray-700 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">Cancel</button>
