...
test("getRealtimeTempMailStats returns expected schema and non-negative numbers", async () => {
  const { getRealtimeTempMailStats } = await import("../src/lib/temp-mail/admin-stats");
  const stats = await getRealtimeTempMailStats();

  assert.ok(stats);
  assert.ok(typeof stats.mailboxes.total === "number" && stats.mailboxes.total >= 0);
  assert.ok(typeof stats.mailboxes.active === "number" && stats.mailboxes.active >= 0);
  assert.ok(typeof stats.mailboxes.expired === "number" && stats.mailboxes.expired >= 0);
  assert.ok(typeof stats.mailboxes.deleted === "number" && stats.mailboxes.deleted >= 0);
  assert.ok(typeof stats.mailboxes.createdToday === "number" && stats.mailboxes.createdToday >= 0);
  assert.ok(typeof stats.emails.total === "number" && stats.emails.total >= 0);
  assert.ok(typeof stats.emails.createdToday === "number" && stats.emails.createdToday >= 0);
  assert.ok(typeof stats.storage.totalBytes === "number" && stats.storage.totalBytes >= 0);
  assert.ok(typeof stats.storage.averageEmailSize === "number" && stats.storage.averageEmailSize >= 0);
  assert.ok(["d1", "mongodb", "hybrid"].includes(stats.source));
});

test("getUserTempMailStats returns valid counts for non-existent and sample users", async () => {
  const { getUserTempMailStats } = await import("../src/lib/temp-mail/admin-stats");
  const stats = await getUserTempMailStats("non_existent_user_id_123");

  assert.ok(stats);
  assert.strictEqual(typeof stats.mailboxes, "number");
  assert.strictEqual(typeof stats.emails, "number");
  assert.ok(stats.mailboxes >= 0);
  assert.ok(stats.emails >= 0);
});

test("getBatchUsersTempMailStats returns a mapped record for all requested user IDs", async () => {
  const { getBatchUsersTempMailStats } = await import("../src/lib/temp-mail/admin-stats");
  const userIds = ["user_1", "user_2", "user_3"];
  const batchStats = await getBatchUsersTempMailStats(userIds);

  assert.ok(batchStats);
  for (const id of userIds) {
    assert.ok(batchStats[id]);
    assert.ok(typeof batchStats[id].mailboxes === "number");
    assert.ok(typeof batchStats[id].emails === "number");
  }
});
